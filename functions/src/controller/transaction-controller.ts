...
export async function importFromCsv(req: Request, res: Response) {
	try {
		...
		for (const object of objects) {
			let amount = object.amount.includes("-") ? parseInt(object.amount) * -1 : parseInt(object.amount)
			let category: FSCategory | undefined = categories.find(c => c.name === object.category.toLowerCase());
			let subCategory: FSSubCategory | undefined = category?.subCategories?.find(sc => sc.name === object.subCategory)
